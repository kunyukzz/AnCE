...
{
    // take a copy of the attachment, renderpass and attachment count.
    out_framebuffer->attachments = ac_allocate_t(sizeof(VkImageView) * attachment_count, MEMTAG_RENDERER);
...
void vulkan_framebuffer_destroy(vulkan_context* context, vulkan_framebuffer* framebuffer)
{
    ...
    {
        ac_free_t(framebuffer->attachments, sizeof(VkImageView) * framebuffer->attachment_count, MEMTAG_RENDERER);

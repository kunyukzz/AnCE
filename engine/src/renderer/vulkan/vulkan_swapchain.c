...
{
    VkResult result =
      vkAcquireNextImageKHR(context->device.logical_device, swapchain->handle, timeout, image_available_semaphore, fence, out_image_index);
    ...
    {
        ACFATAL("Failed to acquire swapchain image!");
        return FALSE;
    }
                              ...
                              u32 present_image_index)
{
    ...
    {
        ACFATAL("Failed to present swapchain image!");
    }
}
...
void create(vulkan_context* context, u32 width, u32 height, vulkan_swapchain* swapchain)
{
    ...
    swapchain_extent.width = ACCLAMP(swapchain_extent.width, min.width, max.width);
    swapchain_extent.height = ACCLAMP(swapchain_extent.height, min.height, max.height);
    ...
    // setup family indices
    if (context->device.graphics_queue_index != context->device.present_queue_index)
    {
        u32 queueFamIndices[] = { (u32)context->device.graphics_queue_index, (u32)context->device.present_queue_index };
        swapchain_create_info.imageSharingMode = VK_SHARING_MODE_CONCURRENT;
        swapchain_create_info.queueFamilyIndexCount = 2;
        swapchain_create_info.pQueueFamilyIndices = queueFamIndices;
    }
    else
    {
    ...
    if (!swapchain->images)
        swapchain->images = (VkImage*)ac_allocate_t(sizeof(VkImage) * swapchain->image_count, MEMTAG_RENDERER);
    if (!swapchain->views)
        swapchain->views = (VkImageView*)ac_allocate_t(sizeof(VkImageView) * swapchain->image_count, MEMTAG_RENDERER);
    ...
    if (!vulkan_device_detect_depth_format(&context->device))
    {
        ...
        ACFATAL("Failed to find a supported image!");
    }
    ...
    ACINFO("Swapchain created successfully");
}
